...
If rotation changed the username, roll back in this order:

1. Set `strict` to `false`, restore a `root` credential while retaining every
   credential still in use, and roll that config through every meta node.
2. Configure every client with the `root` pair. Wait for a normal reconnect
   window and verify that clients reconnect successfully before continuing.
3. Remove the entire `[grpc_auth]` table and roll that config through every meta
   node. The server now applies the old `root`-only username check and ignores
   passwords.
4. Remove client passwords if desired.

Do not remove the auth table before every client uses `root`. Permissive mode
still rejects unknown usernames, and the compatibility path rejects every
username other than `root`.
