...
/// l1 |    b(D) c        e
/// l0 | a  b    c    d              // db
pub async fn build_3_levels_leveled_map() -> anyhow::Result<LeveledMap> {
...
/// ------------------------------------------------------------
/// l0 | a₁  b₂         |  5,2₂ -> b    10,1₁ -> a
pub async fn build_2_levels_leveled_map_with_expire() -> anyhow::Result<LeveledMap> {
    let lm = LeveledMap::default();
...
/// Move the bottom immutable level of `lm` into a persisted db under `base_path`.
///
/// Reaching into the level stack like this is only safe from inside the engine,
/// which is why the helper lives here rather than in the tests that use it.
pub async fn move_bottom_to_db(
    lm: &mut LeveledMap,
    base_path: &str,
    rel_path: &str,
) -> Result<(), std::io::Error> {
    ...
    build_db_from(&mut lm2, base_path, rel_path).await?;

    let persisted = lm2.persisted();
    lm.with_inner(|inner| {
        inner.immutable = std::sync::Arc::new(ImmutableData::new(
...
/// Flush every level of `lm` into a persisted db and make it the only level.
pub async fn build_db_from(
    lm: &mut LeveledMap,
    base_path: &str,
    rel_path: &str,
) -> Result<DB, std::io::Error> {
    ...
    lm.with_inner(|inner| {
        inner.immutable = std::sync::Arc::new(immutable);
    });
