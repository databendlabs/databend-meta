...
impl ImmutableLevels {
    pub fn new_form_iter(immutables: impl IntoIterator<Item = Immutable>) -> Self {
        Self {
    ...
    pub fn indexes(&self) -> Vec<LevelIndex> {
        self.immutables.keys().cloned().collect()
    }

    pub fn newest_to_oldest(&self) -> impl Iterator<Item = &Immutable> {
        self.immutables.values().rev()
    }

    pub fn newest(&self) -> Option<&Immutable> {
        self.immutables.values().next_back()
    }

    pub fn newest_level_index(&self) -> Option<LevelIndex> {
        self.newest().map(|x| *x.level_index())
    }

    pub fn insert(&mut self, level: Immutable) {
    ...
    pub fn remove_levels_upto(&mut self, level_index: LevelIndex) {
        assert!(
    ...
    #[allow(dead_code)]
    pub fn levels_mut(&mut self) -> &mut BTreeMap<LevelIndex, Immutable> {
        &mut self.immutables
    }
    ...
    pub fn get_at_seq<K>(&self, key: K, snapshot_seq: u64) -> SeqMarked<K::V>
    where
        K: MapKey,
