...
async fn test_one_level_upsert_get_range() -> anyhow::Result<()> {
    ...
    assert_eq!(prev, None);
    assert_eq!(result.without_proposed_at(), Some(SeqV::new(1, b("a0"))));
    a.commit().await?;

    let got = sm.get_maybe_expired_kv(&s("a")).await?;
    assert_eq!(got.without_proposed_at(), Some(SeqV::new(1, b("a0"))));

    let mut a = sm.new_applier().await;
    let (prev, result) = a.upsert_kv(&UpsertKV::update("b", b"b0")).await?;
    assert_eq!(prev, None);
    assert_eq!(result.without_proposed_at(), Some(SeqV::new(2, b("b0"))));
    a.commit().await?;

    let got = sm.get_maybe_expired_kv(&s("b")).await?;
    assert_eq!(got.without_proposed_at(), Some(SeqV::new(2, b("b0"))));

    let mut a = sm.new_applier().await;
    let (prev, result) = a.upsert_kv(&UpsertKV::update("a", b"a00")).await?;
    assert_eq!(prev.without_proposed_at(), Some(SeqV::new(1, b("a0"))));
    assert_eq!(result.without_proposed_at(), Some(SeqV::new(3, b("a00"))));
    a.commit().await?;

    let got = sm.get_maybe_expired_kv(&s("a")).await?;
    assert_eq!(got.without_proposed_at(), Some(SeqV::new(3, b("a00"))));

    // get_kv_ref()

    let got = sm
        .get_maybe_expired_kv(&s("a"))
        .await?
        .without_proposed_at();
    assert_eq!(got.seq(), 3);
...
async fn test_two_level_upsert_get_range() -> anyhow::Result<()> {
    ...
    assert_eq!(
        sm.get_maybe_expired_kv(&s("c"))
            .await?
            .without_proposed_at(),
        Some(SeqV::new(4, b("c1")))
    );
    assert_eq!(
        sm.get_maybe_expired_kv(&s("d"))
            .await?
            .without_proposed_at(),
