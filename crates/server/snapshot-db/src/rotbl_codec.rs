...
impl RotblCodec {
    ...
    pub fn encode_range<K, R>(range: &R) -> Result<(Bound<String>, Bound<String>), io::Error>
    where
        K: MapKey,
        K: MapKeyEncode,
        R: RangeBounds<K>,
    ...
    pub fn encode_key_seq_marked<K>(
        key: &K,
    ...
    pub fn encode_key<K>(key: &K) -> Result<String, io::Error>
    where K: MapKeyEncode + ?Sized {
        let mut buf = String::with_capacity(64);
        buf.write_str(K::PREFIX)
            .and_then(|_| buf.write_char('/'))
            .and_then(|_| key.encode(&mut buf))
            .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, e))?;
        Ok(buf)
    }

    /// Decode a key from a string with key-space prefix into [`MapKey`] implementation.
    pub fn decode_key<K>(str_key: &str) -> Result<K, io::Error>
    where
        K: MapKey,
        K: MapKeyEncode,
        K: MapKeyDecode,
