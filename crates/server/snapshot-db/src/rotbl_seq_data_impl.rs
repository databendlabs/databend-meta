...
impl PersistedCodec<SeqData> for SeqData<MetaValue> {
    ...
    fn decode_from(seq_marked: SeqData) -> Result<Self, io::Error> {
...
impl PersistedCodec<SeqData> for SeqData<String> {
    fn encode_to(self) -> Result<SeqData, io::Error> {
    ...
    fn decode_from(seq_data: SeqData) -> Result<Self, io::Error> {
        let marked = SeqData::<(Option<KVMeta>, Vec<u8>)>::decode_from(seq_data)?;
...
mod tests {
    ...
    fn t_string_try_from(marked: SeqData<String>, seq_marked: SeqData) {
        let got: SeqData = marked.clone().encode_to().unwrap();
        assert_eq!(seq_marked, got);

        let got = SeqData::<String>::decode_from(got).unwrap();
    ...
    fn t_try_from(marked: SeqData<MetaValue>, seq_marked: SeqData) {
        ...
        let got = SeqData::decode_from(got).unwrap();
        assert_eq!(marked, got);
    }
    ...
    fn t_invalid(seq_mark: SeqData, want_err: impl ToString) {
        let res = SeqData::<(Option<KVMeta>, Vec<u8>)>::decode_from(seq_mark);
