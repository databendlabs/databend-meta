...
impl From<pb::TxnCondition> for Condition {
    fn from(cond: pb::TxnCondition) -> Self {
        let target = match cond.target {
            Some(txn_condition::Target::Seq(s)) => Operand::Seq(s),
            Some(txn_condition::Target::Value(v)) => Operand::Value(v),
            Some(txn_condition::Target::KeysWithPrefix(n)) => Operand::KeysWithPrefix(n),
            None => Operand::Seq(0),
        };
        ...
        Condition {
            key: cond.key,
            target,
            op: CompareOperator::from_condition_result(cond.expected),
        }
    }
}
