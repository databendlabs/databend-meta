    ...
    <C::LeaderId as RaftLeaderId>::Committed: RaftLeaderId<Term = u64, NodeId = u64> + Ord,
{
    fn from(err: openraft::raft::TransferLeaderError<C>) -> Self {
        ...
        match err {
            TransferLeaderError::VoteChanged { expected, actual } => pb::TransferLeaderError {
                vote_changed: Some(pb::TransferLeaderVoteChanged {
                    expected: Some(expected.into()),
                    actual: Some(actual.into()),
                }),
                log_not_flushed: None,
            },
            TransferLeaderError::LogNotFlushed { expected, actual } => pb::TransferLeaderError {
                vote_changed: None,
                log_not_flushed: Some(pb::TransferLeaderLogNotFlushed {
    ...
    <C::LeaderId as RaftLeaderId>::Committed: RaftLeaderId<Term = u64, NodeId = u64> + Ord,
{
    ...
    fn try_from(value: pb::TransferLeaderError) -> Result<Self, Self::Error> {
        ...
        if let Some(value) = value.vote_changed {
            let expected = value
                .expected
            ...
            return Ok(TransferLeaderError::VoteChanged {
                expected: vote_from_pb::<C>(expected),
                actual: vote_from_pb::<C>(actual),
            });
        }

        if let Some(value) = value.log_not_flushed {
            return Ok(TransferLeaderError::LogNotFlushed {
                expected: value.expected.map(Into::into),
                actual: value.actual.map(Into::into),
            });
        }

        Err(Status::invalid_argument("missing transfer leader error"))
    }
}
