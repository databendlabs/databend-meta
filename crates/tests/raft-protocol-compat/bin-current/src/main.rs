...
//! Standalone binary for backward-compatibility testing of databend-meta (new version).
//!
//! Provides three subcommands:
//! - `serve`  — Start a MetaNode + gRPC server + admin HTTP for `/v1/ctrl/{trigger_snapshot,status}`
...
///
/// Field names form the wire contract for the Python test orchestrator. They
/// intentionally do not mirror `RaftMetrics` so this binary's JSON output is
/// independent of openraft's internal struct evolution.
#[derive(Serialize)]
struct NodeStatus {
    id: u64,
    state: String,
